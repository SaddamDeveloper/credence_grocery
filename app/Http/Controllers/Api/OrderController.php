...
class OrderController extends Controller
{
    public function placeOrder(Request $request)
    {
        ...
        $order_id = 0 ;
        try {
            DB::transaction(function () use($user_id,$delivery_type,$payment_type,$shipping_address_id,$normal_order_status,$bulk_order_status,&$payment_status,&$total_price,&$order_id) {

                $normal_charges = Charges::find(1);
                $express_charges = Charges::find(2);
                $bulk_order_id = null;
                if ($bulk_order_status) {
                    $bulk_order_total = 0;
                    $order = new Order();
                    ...
                    if ($order) {
                        ...
                        foreach ($cart as $key => $cart_data) {
                            ...
                            if ($product->product_type == '2' && ($cart_data->quantity > 5)) {
                                $size_fetch = ProductSize::select('id','mrp','price','size','size_type_id')->where('product_id',$product->id)->where('size',6)->first();
                                $bulk_order_total +=  ($cart_data->quantity*$size_fetch->price);
                                $this->orderDetailsInsert($cart_data,$size_fetch,$user_id,$order->id);
                            }
                        }
                        $order->amount = $bulk_order_total;
                        $order->save();
                        $bulk_order_id = $order->id;
                        $total_price += $bulk_order_total;
                    } else {
                        throw new Exception;
                    }
                }

                if ($normal_order_status) {
                    ...
                    if ($order) {
                        ...
                        foreach ($cart as $key => $cart_data) {
                            $product = $cart_data->product;
                            if ($product->product_type == '2' && ($cart_data->quantity < 6)) {

                                $size_fetch = ProductSize::select('id','mrp','price','size','size_type_id')->where('product_id',$product->id)->where('size',$cart_data->quantity)->first();
                               $product_stock = Product::find($cart_data->product_id);
                               if ($product_stock) {
                                   $product_stock->stock = $product_stock->stock - $cart_data->quantity;
                                   $product_stock->save();
                               }
            ...
            if ($payment_type == '2') {
                ...
                if (!empty($order_update->bulk_order_id)) {
                    $order_bulk = Order::find($order_update->bulk_order_id);
                    $order_bulk->payment_request_id = $orders['id'];
                    $order_bulk->save();
                }
